...
class SceneProposal(SceneEstimationModule):
    ...
    def nms3d(self, scene, strict=False):
        ...
        while remain_obj3d_list:
            ...
            new_obj3d_dict[obj_id] = obj3d #从remain 里面pop出来然后放到新的里面？
    ...
    def pre_nms(self, est_scene_np, gt_scene_np):
        ...
        if est_scene_np['if_log']:
            ...
            cv2.imwrite("nms2d_bdb2d_img",nms2d_bdb2d_img)
    ...
    def finetune_obj3d(self, est_scene, gt_scene):
        lr = 0.01
        est_scene = est_scene.clone()
        gt_scene = None if gt_scene is None else gt_scene.clone()
        # cam_id, est_cam = est_scene['camera'].keys(), 
        for obj_id in est_scene['object'].keys():   
            obj3d_loss = 1. - bdb3d_iou(est_scene['object'][obj_id], gt_scene['object'][obj_id])
            params = []
            params.append({'params': torch.Tensor(est_scene['object'][obj_id]['size']).requires_grad_(), 'lr': lr, 'name': 'size'})
            params.append({'params': torch.Tensor(est_scene['object'][obj_id]['center']).requires_grad_(), 'lr': lr, 'name': 'center'})
            params.append({'params': torch.Tensor(est_scene['object'][obj_id]['rotation_mat']).requires_grad_(), 'lr': lr, 'name': 'rotation_mat'})
            optimizer = torch.optim.Adam(params, lr=0.01)
            optimizer.zero_grad()  # 将所有参数的梯度都置零
            obj3d_loss.backward()        # 误差反向传播计算参数梯度
            optimizer.step()   
    ...
    def generate_scene(self, est_scene, gt_scene):
        ...
        # do class-agnostic NMS if using 2D detection as input 清除重复过量的物体，没有什么别的影响
        # if not self.config['dataset']['gt_affinity']:
        ...
        for i_cam in range(total_cam):
            ...
            # self.finetune_obj3d(best_scene_np, gt_scene_np)
