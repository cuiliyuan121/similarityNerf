...
class SceneOptimizationDataset(Front3DDataset):
    def get_data(self, input_scene, gt_scene):
        ...
        # input_data['object'] = dict((key, value) for key, value in input_data['object'].items() if key in gt_obj_single_ids) # data/understanding 读入的数据没有object 
...
class SceneOptimization(SceneEstimationModule):
    ...
    def single_object_volume_rendering(self, camera, latent_code, object3d, rays, batch_size, z_sample,
        ...
        return self.autoencoder_nerf.decoder.volume_rendering(
            ...
            xyz[None, ...],
            z_vals[None, ...],
            latent_code,  #object3d['latent_code']
            self.config['z_sample'],
            extra_model_outputs=['attention_values'],
            white_background=white_background)

    def render_object_distance(self, camera, latent_code, object3d_list, num_iter, rays=None, get_distance_map=False):
        if rays is None:
        ...
        for obj_idx, object3d in enumerate(object3d_list):
            ...
            dis = self.autoencoder_nerf.decoder.decoder.sphere_tracing_surface_distance(
                latent_code, rays_o, viewdir, dis_init=dis_near, dis_far=dis_far, num_iter=num_iter, extra_model_outputs=['attention_values'])
    ...
    def sample_rays_for_scene(self, camera, latent_code, object3d_list, random_ray_sample=None, object2d_list=None,
        ...
        with torch.no_grad():
            self.render_object_distance(camera, latent_code, object3d_list, self.config['sphere_tracing_iter'], rays=rays)
    ...
    def object_novel_view_synthesis(self, camera, latent_code, object3d_list, batch_size, z_sample,
                                    rays=None, backward_fcn=None, get_img=False, patch_sizes=None, detach_sample_point=True):
        if rays is None:
            rays = self.sample_rays_for_scene(camera, latent_code, object3d_list)
        ...
        for obj_idx, object3d in enumerate(object3d_list):
            ...
            obj_results = self.single_object_volume_rendering(
                camera, latent_code, object3d, obj_rays, batch_size, z_sample,
    ...
    def init_latent_code(self, est_scene, overwrite=True):
        if self.code_embedding == 'mean_latent_code':
            if not overwrite and 'latent_code' in est_scene:
                return 
            for object3d in est_scene['object'].values():
                est_scene['latent_code'] = self.autoencoder_nerf.decoder.mean_latent_code(object3d['category_id'])
                break
        else:
        ...
        if camera_dict is None:
            for camera in est_scene['camera'].values():
                results = self.object_novel_view_synthesis(
                    camera, est_scene['latent_code'], list(est_scene['object'].values()), self.config['batch_size'], self.config['z_sample'], get_img=True)
            ...
            with torch.no_grad():
                ...
                if 'video' in scene_optim_vis.vis_types:
                    for camera_id, camera in scene['camera'].items():
                        results = self.object_novel_view_synthesis(
                            camera, scene['latent_code'], list(scene['object'].values()), self.config['batch_size'], self.config['z_sample'], get_img=True)
            ...
            with tqdm(total=num_opts, desc='Optimizing scene', leave=False) as pbar:
                while i < num_opts:
                    ...
                    for camera_id, camera in est_scene['camera'].items():
                        ...
                        results = self.object_novel_view_synthesis(
                            camera, est_scene['latent_code'], obj3d_list, self.config['batch_size'], self.config['z_sample'],
    ...
    def test_step(self, batch, batch_idx):
        ...
        if not self.config['test']['skip_prediction']:
            ...
            #est_scene_novel['latent_code'] = est_scene['latent_code']
       ...
       # self.eval_scene(est_scene, gt_scene, est_scene_novel, gt_scene_novel) 
