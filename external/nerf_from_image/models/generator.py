...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_generator_encoder = use_generator_encoder
        self.num_classes = num_classes

        c_dim = 512 if num_classes else 0
        z_dim = latent_dim
        w_dim = 512
        if use_generator_encoder:
    ...
    def forward(self,
        ...
        if self.use_viewdir and viewdir is not None:
            viewdir_mapper_closure = self.viewdir_mapper(viewdir.unsqueeze(-2))
        ...
        def sampler(x_in, request_sampler_outputs=['sigma', 'rgb']):
            ...
            if 'normals' in request_sampler_outputs:
                x_grad, = torch.autograd.grad(density_or_distance[...,
                                              ...
                                              x_in,
                                              create_graph=False)
                sampler_outputs['normals'] = F.normalize(x_grad, dim=-1).view(*x_in.shape[:-1], -1)
            ...
            if 'sdf_distance' in request_sampler_outputs:
                sampler_outputs['sdf_distance'] = density_or_distance.view(*x_in.shape[:-1], -1) # bs, n_rays, n_sample, 1
            ...
            if 'sigma' in request_sampler_outputs:
                if self.use_sdf:
                    ...
                    sampler_outputs['sigma'] = sigma_final.view(*x_in.shape[:-1], -1)
                else:
                    # Standard NeRF density
            ...
            if 'coords' in request_sampler_outputs:
                sampler_outputs['coords'] = x_in.view(*x_in.shape[:-1], -1)
            ...
            if 'rgb' in request_sampler_outputs or 'semantics' in request_sampler_outputs:
                ...
                if 'semantics' in request_sampler_outputs:
                    assert self.attention_values > 0
                    sampler_outputs['semantics'] = attention_probs.view(*x_in.shape[:-1], -1)

                if 'rgb' in request_sampler_outputs:
                    if self.attention_values > 0:
                        rgb = torch.matmul(attention_probs, attention_values)

                    rgb = rgb.view(*x_in.shape[:-1], -1)
    ...
    def sphere_tracing_surface_distance(self, code, rays_o, viewdir,dis_init=0.0, dis_far=6.0,
                                        num_iter=20, extra_model_outputs=[], extra_model_inputs={}):
        viewdirs = None
        model_outputs = self(viewdirs, code, ['sampler'] + extra_model_outputs, extra_model_inputs)
        radiance_field_sampler = model_outputs['sampler']
        del model_outputs['sampler']
        dis_preds = dis_init.clone() if isinstance(dis_init, torch.Tensor) else \
            torch.ones([*rays_o.shape[:-1]], device=rays_o.device) * dis_init
        for _ in range(num_iter):
            xyz = rays_o + viewdir * dis_preds[..., :, None]
            dis_preds += radiance_field_sampler(xyz[None, :, None, :], request_sampler_outputs=['sdf_distance'])['sdf_distance'].flatten()

        dis_preds[dis_preds > dis_far] = float('inf')
        too_close = dis_preds < dis_init
        dis_preds[too_close] = dis_init[too_close]
