...
def main():
    ...
    if not distributed:
        ...
        dir_name =cfg.data.test.data_root.split('/')[-1]
        npy_dirs = glob(os.path.join(args.show_dir, '*.npy'))
        for dir in npy_dirs:
            depthmap = normalization(np.load(dir)[0]) #使用numpy载入npy文件
            depthmap= depthmap.astype(int)
            print(depthmap.shape)
            file_name = ((dir.split('.')[0]).split('/')[-1]).split('-')[0]
            cv2.imwrite(args.show_dir + '/' + dir_name + '/'+ file_name+'-depth.png', depthmap)
    else:
