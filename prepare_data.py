...
def link_depth_output(args):
    ...
    output_root = default_output_root(args.output_dir, 'scene-monocular_depth', args.debug)
    save_splits(output_root, splits)
    ...
    for scene_id in tqdm(scene_ids, desc='Linking depth outputs'):
        ...
        for file_dir in glob(os.path.join(input_scene_dir, '*')):
            if not os.path.splitext(os.path.basename(file_dir))[0].endswith('depth'):
                os.link(file_dir, os.path.join(output_scene_dir, os.path.basename(file_dir)))
...
def generate_scene_dataset_from_detection_result(args):
    ...
    # load splits
    print('Loading scene ids...')
    scene_root = default_output_root(args.data_dir, 'scene-monocular_depth', args.debug)
    splits = load_splits(scene_root)

    output_root = default_output_root(args.output_dir, 'scene-detector', args.debug)
    save_splits(output_root, splits)
    ...
    for scene_id in tqdm(scene_ids, desc='Generating dataset'):
        ...
        for cam_id, camera in input_scene['camera'].items():
            image = dict(camera['image'])
            os.link(image['depth'], os.path.join(output_scene_dir, os.path.basename(image['depth'])))
            os.link(image['color'], os.path.join(output_scene_dir, os.path.basename(image['color'])))

        # split detection results
        image_segm_results = defaultdict(list)
        for segm_result in segm_results:
            image_segm_results[segm_result['image_id']].append(segm_result)
        ...
        for cam_id, image_idx in scene_cam_idx[scene_id].items():
            ...
            obj2d_annotations = {}
            for obj_id, segm_result in enumerate(image_segm_result):
                if args.nyuv2 and segm_result['category_id'] not in coco_id2category:
                    continue
                segmentation = mask_util.decode(segm_result['segmentation']).astype(bool)
                obj2d_annotations[obj_id] = {
                    'category': segm_result['category_id'],
                    'bdb2d': np.array(segm_result['bbox']).round().astype(int),
                    'score': segm_result['score'],
                    'segmentation': segmentation,
                    'area': blenderproc_utils.calc_binary_mask_area(segmentation),
                }
                if args.nyuv2:
                    obj2d_annotations[obj_id]['category'] = coco_id2category[segm_result['category_id']]
...
def generate_scene_understanding_dataset(args):
    ...
    for i in tqdm(range(len(dataset)), desc='Generating dataset'):
        ...
        scene_id = input_scene['uid']  #这里好像读不出来 为啥捏
...
def main():
    ...
    args.additional_args = ['additional_args', 'job', 'overwrite', 'processes', 'retry', 'single_gpu']
